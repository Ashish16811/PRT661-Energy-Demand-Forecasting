...
# 5. BOM weather
# ------------------------------------------------------------------
def fetch_bom_weather(regions, output_dir):
    ...
    for region in regions:
        ...
        for label, code in products.items():
            p_c = None
            try:
                r = requests.get("http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av"
                                 f"?p_nccObsCode={code}&p_display_type=dataFile"
                                 f"&p_stn_num={station}",
                                 headers=BOM_HEADERS, timeout=30)
                r.raise_for_status()
                match = re.search(r"p_c=(-?\d+)", r.text)
                if match:
                    p_c = match.group(1)
            except Exception as e:
                log.warning(f"[BOM] {region}/{label}: could not resolve p_c: {e}")

            try:
                r = requests.get("http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av"
                                 f"?p_nccObsCode={code}&p_display_type=dailyDataFile"
                                 f"&p_startYear=&p_c={p_c or ''}&p_stn_num={station}",
                                 headers=BOM_HEADERS, timeout=60)
                r.raise_for_status()
                path = os.path.join(output_dir, f"bom_{region}_{label}_station{station}.csv")
                with open(path, "wb") as f:
                    f.write(r.content)
                log.info(f"[BOM] {region}/{label} -> {path}")
                written[f"{region}_{label}"] = path
            except Exception as e:
                log.error(f"[BOM] {region}/{label} failed: {e}")
            time.sleep(1)
...
def main():
    ...
    p.add_argument("--regions", nargs="+", default=NEM_REGIONS,
                   help="NEM regions for demand data (default: all five)")
    p.add_argument("--wa-tail-days", type=int, default=14,
                   help="Max days of WA daily top-up files to request (default 14)")
    ...
    p.add_argument("--skip-bom", action="store_true")
    ...
    # Demand regions come from --regions; feature regions always add WA and NT.
    demand_regions = args.regions
    feature_regions = list(dict.fromkeys(list(args.regions) + ["WA", "NT"]))

    def run_step(label, fn, *fn_args, **fn_kwargs):
        """
        Run one pipeline step in isolation.

        Each step is independent, so a source that is slow, unreachable or
        malformed logs an error and the run continues. Previously an
        exception or a stall in an early step meant the later steps - notably
        dispatch and pre-dispatch - never executed at all.
        """
        log.info(label)
        try:
    ...
    # Order matters. The dashboard serves a short rolling window, so it is
    # fetched before the slower archive and WA steps - a snapshot missed is
    # gone, whereas the archives are static and can be retried any time.
    if not args.skip_dashboard:
        run_step("1/6 Dashboard API -> dispatch/ and predispatch/",
                 fetch_dispatch_and_predispatch,
                 demand_regions, str(dirs["dispatch"]), str(dirs["predispatch"]),
                 str(dirs["snapshots"]) if args.archive_snapshots else None)

    if not args.skip_archive:
        run_step("2/6 Monthly NEM archive (deep history)",
                 fetch_price_and_demand,
                 args.start, args.end, demand_regions, str(dirs["archive"]))

    if not args.skip_wa:
        run_step("3/6 WA operational demand (native source intervals)",
                 fetch_wa_demand,
                 args.start, args.end, str(dirs["archive"]),
                 tail_days=args.wa_tail_days)

        run_step("4/6 WA 2026 market requirements",
                 fetch_wa_market_requirements_2026, str(dirs["requirements"]))

    if not args.skip_bom:
        run_step("5/6 BOM daily weather (including WA and NT)",
                 fetch_bom_weather, feature_regions, str(dirs["bom"]))

    if not args.skip_holidays:
        run_step("6/6 Public holidays (including WA and NT)",
                 fetch_public_holidays,
                 args.start, args.end, feature_regions, str(dirs["holidays"]))
